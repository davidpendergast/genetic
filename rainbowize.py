...
def transform_rgb(surface: pygame.Surface, weights=((1, 0, 0),
                                                    (0, 1, 0),
                                                    (0, 0, 1))):
    """Linearly transforms the RGB color channels of a surface.

        If the original image's color in a voxel is R, G, B, and weights are:
            ((r1, r2, r3),
             (g1, g2, g3),
             (b1, b2, b3))

        Then the output color R', G', B' is given by:
            R' = r1 * R + r2 * G + r3 * B
            G' = g1 * R + g2 * G + g3 * B
            B' = b1 * R + b2 * G + b3 * B

        For example, these weights would swap the R and G channels:
            ((0, 1, 0),
            (1, 0, 0),
            (0, 0, 1))

        And these weights would give a luminosity-respecting grayscale image:
            ((0.299, 0.587, 0.114),
             (0.299, 0.587, 0.114),
             (0.299, 0.587, 0.114))
    ...
    res = surface.copy()
    red_pixels = pygame.surfarray.pixels_red(res)
    red_pixels[:] = (px[0] * rw[0] + px[1] * rw[1] + px[2] * rw[2]).astype(dtype='uint16')

    green_pixels = pygame.surfarray.pixels_green(res)
    green_pixels[:] = (px[0] * gw[0] + px[1] * gw[1] + px[2] * gw[2]).astype(dtype='uint16')

    blue_pixels = pygame.surfarray.pixels_blue(res)
    blue_pixels[:] = (px[0] * bw[0] + px[1] * bw[1] + px[2] * bw[2]).astype(dtype='uint16')
...
if __name__ == "__main__":
    ...
    while True:
        ...
        rainbow_frog_img = apply_rainbow(
            frog_img,
            offset=4.2, # elapsed_time_ms / animation_period_ms / 10,
            bands=1.2
        )
        ...
        t = (1 + math.sin(elapsed_time_ms / animation_period_ms)) / 2

        shifted_frog_img = transform_rgb(
            rainbow_frog_img,
            weights=(interp(t, (1, 0, 0), (0.299, 0.587, 0.114)),
                     interp(t, (0, 1, 0), (0.299, 0.587, 0.114)),
                     interp(t, (0, 0, 1), (0.299, 0.587, 0.114))))

        scr_w, scr_h = screen.get_size()
        screen.blit(frog_img, (scr_w / 6 - frog_img.get_width() / 2, scr_h / 2 - frog_img.get_height() / 2))
        screen.blit(rainbow_frog_img, (3 * scr_w / 6 - rainbow_frog_img.get_width() / 2,
                                       scr_h / 2 - rainbow_frog_img.get_height() / 2))
        screen.blit(shifted_frog_img, (5 * scr_w / 6 - shifted_frog_img.get_width() / 2,
                                       scr_h / 2 - shifted_frog_img.get_height() / 2))
